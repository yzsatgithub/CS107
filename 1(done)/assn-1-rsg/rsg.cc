...
string chooseProduction(string nonterminal, map<string, Definition>& grammar){
    string result;
    Production sentence;
    sentence = grammar[nonterminal].getRandomProduction();   // choose production
    for(Production::iterator curr = sentence.begin(); curr != sentence.end(); curr++){
        if((*curr)[0] == '<'){
            result += chooseProduction(*curr, grammar);
        }
        else{
            result += " ";
            result += *curr;
...
int main(int argc, char *argv[])
{
  ...
  for(int i = 0; i < 3; i++){
    ...
    string sentence = chooseProduction("<start>", grammar);
